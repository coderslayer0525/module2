...
public class ThuAnThitRepository implements IThuAnThitRepository{
    ...
    public boolean update(int maSo, ThuAnThit thuAnThit) {
        for (int i = 0;i < thuAnThitArrayList.size();i++){
            if (thuAnThitArrayList.get(i).getMaSo()== new ThuAnThit().getMaSo()){
    ...
    public ThuAnThit findByName(String name) {
        for (int i = 0; i < thuAnThitArrayList.size(); i++)
            if (thuAnThitArrayList.equals(name)){
